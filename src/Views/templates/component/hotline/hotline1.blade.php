...
@if (!empty($destop) || !empty($mobile))
    <div id="hotline" class="hotline2" style="--bottom: {{ $bottom }}px;--left: {{ $left }};--right: {{ $right }};">
        @foreach ($hotline as $hotline)
            <a class="btn-phone btn-frame text-decoration-none" href="tel:{{ $hotline }}" style="--background: #{{ $background }};">
                <div class="animated infinite zoomIn kenit-alo-circle"></div>
                <div class="animated infinite pulse kenit-alo-circle-fill"></div>
                <i><img onerror="this.src='assets/images/noimage.png';" src="{{ $photo }}" alt="hotline" title="hotline" width="35" /></i>
            </a>
        @endforeach
    </div>
