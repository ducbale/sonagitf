    ...
    <div class="tab-content" id="custom-tabs-three-tabContent-lang">
        @foreach (config('app.langs') as $k => $v)
            <div class="tab-pane fade show {{ $k == 'vi' ? 'active' : '' }}" id="tabs-lang-{{ $k }}"
                ...
                @if (!empty($content1))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="content1{{ $k }}">Hướng dẫn
                            ({{ $k }}):</label>
                        <textarea class="form-control for-seo text-sm {{ !empty($content1_cke) ? 'form-control-ckeditor' : '' }}"
                            name="data[content1{{ $k }}]" id="content1{{ $k }}" rows="5"
                            placeholder="Hướng dẫn ({{ $k }})">{{ !empty(Flash::has('content1' . $k)) ? Flash::get('content1' . $k) : @$item['content1' . $k] }}</textarea>
                    </div>
                @endif
                @if (!empty($parameter))
                    <div class="form-group last:!mb-0">
